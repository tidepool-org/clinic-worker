	...
	Context("With EHR Match Response", func() {
		...
		Describe("PopulateSummaryStatistics", func() {
			It("populates cgm and bgm observations", func() {
				...
				Expect(observations).To(ContainObservation(Observation{"TIME_IN_EXTREME_HIGH_CGM", "0.0436", "Numeric", &expectedPercentageUnits, "The percentage of time during the reporting period that CGM data was greater than or equal to 350 mg/dl"}))
			...
			It("populates cgm and bgm observations with N/A when summaries are empty", func() {
				...
				Expect(observations).To(ContainObservation(Observation{"REPORTING_PERIOD_END_CGM", "NOT AVAILABLE", "DateTime", nil, "CGM Reporting Period End"}))
				Expect(observations).To(ContainObservation(Observation{"REPORTING_PERIOD_START_CGM_DATA", "NOT AVAILABLE", "DateTime", nil, "CGM Reporting Period Start Date of actual Data"}))
				...
				Expect(observations).To(ContainObservation(Observation{"GLUCOSE_MANAGEMENT_INDICATOR", "NOT AVAILABLE", "Numeric", nil, "CGM Glucose Management Indicator during reporting period"}))
				Expect(observations).To(ContainObservation(Observation{"TIME_BELOW_RANGE_VERY_LOW_CGM", "NOT AVAILABLE", "Numeric", &expectedPercentageUnits, "CGM Time in Level 2 Hypoglycemia: <Time below range (TBR-VL): % of readings and time <54 mg/dL (<3.0 mmol/L)"}))
				Expect(observations).To(ContainObservation(Observation{"TIME_BELOW_RANGE_LOW_CGM", "NOT AVAILABLE", "Numeric", &expectedPercentageUnits, "CGM Time in Level 1 Hypoglycemia: Time below range (TBR-L): % of readings and time 54–69 mg/dL (3.0–3.8 mmol/L)"}))
				Expect(observations).To(ContainObservation(Observation{"TIME_IN_RANGE_CGM", "NOT AVAILABLE", "Numeric", &expectedPercentageUnits, "CGM Time in Range: Time in range (TIR): % of readings and time 70–180 mg/dL (3.9–10.0 mmol/L)"}))
				Expect(observations).To(ContainObservation(Observation{"TIME_ABOVE_RANGE_HIGH_CGM", "NOT AVAILABLE", "Numeric", &expectedPercentageUnits, "CGM Time in Level 1 Hyperglycemia: Time above range (TAR-H): % of readings and time 181–250 mg/dL (10.1–13.9 mmol/L)"}))
				Expect(observations).To(ContainObservation(Observation{"TIME_ABOVE_RANGE_VERY_HIGH_CGM", "NOT AVAILABLE", "Numeric", &expectedPercentageUnits, "CGM Level 2 Hyperglycemia: Time above range (TAR-VH): % of readings and time >250 mg/dL (>13.9 mmol/L)"}))
				Expect(observations).To(ContainObservation(Observation{"TIME_IN_EXTREME_HIGH_CGM", "NOT AVAILABLE", "Numeric", &expectedPercentageUnits, "The percentage of time during the reporting period that CGM data was greater than or equal to 350 mg/dl"}))
				Expect(observations).To(ContainObservation(Observation{"REPORTING_PERIOD_START_SMBG", "NOT AVAILABLE", "DateTime", nil, "SMBG Reporting Period Start"}))
				Expect(observations).To(ContainObservation(Observation{"REPORTING_PERIOD_END_SMBG", "NOT AVAILABLE", "DateTime", nil, "SMBG Reporting Period End"}))
				Expect(observations).To(ContainObservation(Observation{"REPORTING_PERIOD_START_SMBG_DATA", "NOT AVAILABLE", "DateTime", nil, "SMBG Reporting Period Start Date of actual Data"}))
				Expect(observations).To(ContainObservation(Observation{"CHECK_RATE_READINGS_DAY_SMBG", "NOT AVAILABLE", "Numeric", nil, "Average Numeric of SMBG readings per day during reporting period"}))
				Expect(observations).To(ContainObservation(Observation{"AVERAGE_SMBG", "NOT AVAILABLE", "Numeric", nil, "SMBG Average Glucose during reporting period"}))
				Expect(observations).To(ContainObservation(Observation{"READINGS_BELOW_RANGE_VERY_LOW_SMBG", "NOT AVAILABLE", "Numeric", nil, "SMBG Level 2 Hypoglycemia Events: Number of readings <54 mg/dL (<3.0 mmol/L) during reporting period"}))
				Expect(observations).To(ContainObservation(Observation{"READINGS_ABOVE_RANGE_VERY_HIGH_SMBG", "NOT AVAILABLE", "Numeric", nil, "SMBG Level 2 Hyperglycemia: Number of readings above range (TAR-VH) time >250 mg/dL (>13.9 mmol/L) during reporting period"}))
			})
