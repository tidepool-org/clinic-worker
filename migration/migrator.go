...
func (m *migrator) updateMigrationsStatus(ctx context.Context, migration *Migration, status string) error {
	...
	resp, err := m.clinics.UpdateMigrationWithResponse(ctx, *migration.clinic.Id, migration.legacyClinicianUserId, body)
	if err != nil {
		return err
	}
...
func (m *migrator) sendMigrationEmail(ctx context.Context, migrationContext *Migration, patient *clinics.Patient) error {
	if patient.Email == nil || *patient.Email == "" {
		m.logger.Infof("Skipping sending of migration email to user %s because email address is empty", *patient.Id)
		return nil
	}

	m.logger.Infof("Sending migration email to user %s", *patient.Id)
